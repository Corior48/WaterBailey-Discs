...
public class WaterBaileyDiscs {
    ...
    // Create a Deferred Register to hold Blocks which will all be registered under the "waterbaileydiscs" namespace
    public static final DeferredRegister.Blocks BLOCKS = DeferredRegister.createBlocks(MODID);
    // Create a Deferred Register to hold Items which will all be registered under the "waterbaileydiscs" namespace
    public static final DeferredRegister.Items ITEMS = DeferredRegister.createItems(MODID);
    // Create a Deferred Register to hold CreativeModeTabs which will all be registered under the "waterbaileydiscs" namespace
    public static final DeferredRegister<CreativeModeTab> CREATIVE_MODE_TABS = DeferredRegister.create(Registries.CREATIVE_MODE_TAB, MODID);

    // Creates a new Block with the id "waterbaileydiscs:example_block", combining the namespace and path
    // public static final DeferredBlock<Block> EXAMPLE_BLOCK = BLOCKS.registerSimpleBlock("example_block", BlockBehaviour.Properties.of().mapColor(MapColor.STONE));
    // Creates a new BlockItem with the id "waterbaileydiscs:example_block", combining the namespace and path
    // public static final DeferredItem<BlockItem> EXAMPLE_BLOCK_ITEM = ITEMS.registerSimpleBlockItem("example_block", EXAMPLE_BLOCK);

    // Creates a new food item with the id "waterbaileydiscs:example_id", nutrition 1 and saturation 2
    ...
    public WaterBaileyDiscs(IEventBus modEventBus, ModContainer modContainer) {
        ...
        // Note that this is necessary if and only if we want *this* class (WaterBaileyDiscs) to respond directly to events.
