...
public class MusicBlockMenu extends AbstractContainerMenu {
    ...
    private int getSelectedXpCost() {
        ...
        if (selected < 0 || selected >= net.corior48.waterbailey_discs.utils.DiscOptions.size()) {
            return 0;
        }
        return net.corior48.waterbailey_discs.utils.DiscOptions.getXpCost(selected);
    }
    ...
    public boolean clickMenuButton(Player player, int id) {
        ...
        int size = net.corior48.waterbailey_discs.utils.DiscOptions.size();
        if (size <= 0) {
            return false;
        }
